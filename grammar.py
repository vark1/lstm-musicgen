from music21 import scale, note, chord, interval, stream
import copy, random

def __is_diatonic(chord, note):
...
# approach note is either diatonic from below or above OR chromatic from below or above (we're using chromatic only for simplicity)
def __is_approach_note(chord, note):
    for chordTone in chord.pitches:
        chromaticAbove = chordTone.transpose(1)
        chromaticBelow = chordTone.transpose(-1)
        if (note.name == chromaticBelow.name or note.name == chromaticBelow.getEnharmonic().name or
            note.name == chromaticAbove.name or note.name == chromaticAbove.getEnharmonic().name):
                return True
    return False
...
def parse_melody(fullMeasureNotes, fullMeasureChords):
    ...
    measureStartTime = measure[0].offset - (measure[0].offset%4)    # the offset for measure's start, e.g. 476.0.
    ...
    for ix, nr in enumerate(measure):
        ...
        if isinstance(nr, note.Rest):
            ...
            elementType = 'C'
        elif __is_diatonic(lastChord, nr):
            elementType = 'S'
        elif __is_approach_note(lastChord, nr):
            elementType = 'A'
        else:
            elementType = 'X'
    ...
    for ix, grammarElement in enumerate(m1_grammar.split(' ')):
        ...
        if (len(terms) == 2):
            ...
            if terms[0] == 'C':
                ...
                for i in range(0, numNotes):
                    currNote = note.Note(lowPitch.transpose(i).simplifyEnharmonic())
                    if __is_diatonic(lastChord, currNote):
                ...
                for i in range(0, numNotes):
                    currNote = note.Note(lowPitch.transpose(i).simplifyEnharmonic())
                    if __is_approach_note(lastChord, currNote):
